...
const WORKER_URL = process.env.WORKER_URL || "https://epstein-files.rhys-669.workers.dev";
...
async function main() {
  try {
    ...
    console.log(`  Total size: ${(JSON.stringify(files).length / 1024 / 1024).toFixed(2)} MB`);
  } catch (error) {
